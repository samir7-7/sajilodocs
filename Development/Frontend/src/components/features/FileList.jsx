...
const FileList = ({ files, viewMode = 'list', onEdit, onDelete, onContextMenu }) => {
  const getFileIcon = (file) => {
    ...
    if (type.includes('audio') || name.match(/\.(mp3|wav|ogg)$/i)) {
      return <FileAudio className="text-indigo-500" size={20} />;
    }
  ...
  const formatFileSize = (bytes) => {
    ...
    if (typeof bytes === 'string' && /[a-zA-Z]/.test(bytes)) {
      ...
      if (!isNaN(numPart)) return Math.ceil(numPart) + ' ' + unitPart;
      return bytes;
    }
        ...
        {files.map((file) => (
          ...
          >
            <div className="flex-1 flex items-center gap-4 min-w-0">
              <div className="flex-shrink-0 p-2.5 bg-white border border-slate-100 rounded-xl group-hover:border-[#4f46e5]/30 transition-all shadow-sm">
                {getFileIcon(file)}
              </div>
              <div className="min-w-0">
                <h4 className="font-semibold text-slate-900 truncate group-hover:text-[#4f46e5] transition-colors" title={file.name}>
                  {file.name}
                </h4>
                <p className="lg:hidden text-[11px] text-slate-400 font-medium mt-0.5">
                  {formatFileSize(file.size)} • Up: {formatDate(file.created_at)}
                </p>
              </div>
            </div>
            <div className="hidden lg:flex w-40 items-center gap-1.5 flex-wrap">
              {file.tags?.slice(0, 2).map((tag, idx) => (
                <span key={idx} className="px-2 py-0.5 bg-slate-50 text-slate-500 border border-slate-100 text-[10px] font-bold rounded uppercase tracking-tighter">
                  {tag}
                </span>
              ))}
            </div>
            <div className="hidden lg:block w-24 text-sm font-medium text-slate-500">{formatFileSize(file.size)}</div>
            <div className="hidden lg:block w-36 text-xs font-semibold text-slate-400">{formatDate(file.created_at)}</div>
            <div className="hidden lg:block w-36 text-sm font-bold text-slate-600">{formatDate(file.updated_at || file.created_at)}</div>
            <div className="w-20 flex justify-end items-center gap-1">
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  if (file.file_url) {
                    const link = document.createElement('a'); link.href = file.file_url;
                    link.download = file.name; document.body.appendChild(link);
                    link.click(); document.body.removeChild(link);
                  }
                }}
                className="p-2 text-slate-400 hover:text-[#4f46e5] rounded-lg opacity-0 group-hover:opacity-100 transition-all"
                title="Download"
              ...
              <button
                ...
                className="p-2 text-slate-400 hover:text-red-600 rounded-lg opacity-0 group-hover:opacity-100 transition-all"
                title="Delete"
